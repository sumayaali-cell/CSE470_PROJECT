...
exports.getDonatedItems = async (req, res) => {
  try {
    ...
    // Check the most recent item date for debugging
    const latestItem = await postModel.findOne().sort({ date: -1 });
    console.log("Latest item date in DB:", latestItem?.date);

    const items = await postModel.find({
      // 👉 If you want OLDER than 60 days:
      // date: { $lt: sixtyDaysAgo },
      // 👉 If you want WITHIN last 60 days:
      date: { $lte: sixtyDaysAgo },
      claimed: false
    }).sort({ date: -1 });
