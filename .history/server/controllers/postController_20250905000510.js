...
exports.getDonatedItems = async (req, res) => {
  try {
    ...
    // If you want items OLDER than 60 days → use $lt
    // If you want items WITHIN the last 60 days → use $gte

    const items = await postModel.find({
      date: { $lt: sixtyDaysAgo }, // change to $gte if you want last 60 days
      claimed: false
    }).sort({ date: -1 });
